(() => {
  ...
  function text(page, value, x, y, size, fonts, bold = false) {
    page.drawText(String(value ?? ''), { x, y, size, font: bold ? fonts.bold : fonts.regular, color: PDFLib.rgb(0.08, 0.11, 0.17) });
  }
  ...
  async function exportFillablePDF() {
    ...
    const core = [['hull', `Hull: ${ship.hull} Class`], ['bridge', `Bridge: ${ship.bridge}`], ['power_plant', `Power Plant: ${ship.powerPlant}`], ['shield', `Shield: ${ship.shield}`]];
    ...
    const stats = [['current_hull', `Hull HP: ${ship.currentHull} / ${c.h.hp}`], ['current_shield', `Shield HP: ${ship.currentShield} / ${c.shieldMax}`], ['current_power', `Power: ${ship.currentPower} / ${c.p.capacity}`], ['cargo_used', `Cargo: ${ship.cargoUsed} / ${c.cargo} tonnes`], ['combat_round', `Combat Round: ${ship.combatRound}`], ['ftl_countdown', `FTL Countdown: ${ship.ftlCountdown ?? '-'}`]];
    ...
    const caps = [`Command Points: ${c.b.cp}`, `Shield Factor: ${c.h.shieldFactor}`, `Cargo Capacity: ${c.cargo} tonnes`, `Crew Quarters: ${c.h.crew}`, `Staterooms: ${c.h.staterooms}`, `Passenger Capacity: ${c.passengers}`, `Communication Range: ${c.comm} parsecs`, `Monthly Maintenance: ${fmtMoney(c.maintenance)}`];
  ...
  function installButton() {
    ...
    const btn = document.createElement('button'); btn.className = 'btn'; btn.dataset.pdfExport = 'true'; btn.textContent = 'Export fillable PDF';
    btn.addEventListener('click', () => exportFillablePDF().catch(err => { console.error('Astrovoyage PDF export error:', err); alert(`PDF export failed: ${err.message || 'Unknown error'}. Please refresh and try again.`); }));
