  ...
  function renderBuilder(){
    ...
    if(step===0){
      ...
      body=`<div class="builder-card"><h2>Hull & Core Systems</h2><div class="builder-grid">
        ...
        <label class="builder-field"><span>Bridge</span><select id="b-bridge">${Object.entries(BRIDGES).map(([k,v])=>`<option value="${k}" ${k===ship.bridge?'selected':''}>${k} Bridge · ${v.cost} · ${v.cp} CP · Initiative ${v.initiative}</option>`).join('')}</select></label>
