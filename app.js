...
const BRIDGES={
  Basic:{cost:'1MCr',cp:4,installation:10,initiative:0},Expert:{cost:'2MCr',cp:5,installation:15,initiative:1},
  Companion:{cost:'3.5MCr',cp:6,installation:20,initiative:5},Master:{cost:'5MCr',cp:7,installation:25,initiative:2},Immortal:{cost:'7.5MCr',cp:8,installation:30,initiative:3}
};
...
function fresh(){return {name:'Unnamed Vessel',captain:'',hull:'Faber',bridge:'Basic',powerPlant:'Basic',shield:'Buckler',currentHull:75,currentShield:100,currentPower:100,cargoUsed:0,systemDamage:{Cargo:0,Helm:0,Weapons:0,Engineering:0,LSC:0,Power:0},mods:[],weapons:[],notes:'',combatRound:1,ftlCountdown:null,commandPoints:4,initiative:0,updatedAt:null}}
...
function calc(){
  ...
  const modUsed=ship.mods.reduce((a,m)=>a+(MODS[m]?.size||0),0);
  const extraPowerUsed=(ship.extraPowerPlants||[]).reduce((a,p)=>a+(POWER[p]?.size||0),0);
  const used=modUsed+extraPowerUsed;
...
function normalise(){const b=BRIDGES[ship.bridge]||BRIDGES.Basic;if(ship.commandPoints===undefined||ship.commandPoints===null||Number.isNaN(Number(ship.commandPoints)))ship.commandPoints=Number(b.cp||0);if(ship.initiative===undefined||ship.initiative===null||Number.isNaN(Number(ship.initiative)))ship.initiative=Number(b.initiative||0);const c=calc();ship.currentHull=Math.min(Math.max(0,Number(ship.currentHull)||0),c.h.hp);ship.currentShield=Math.min(Math.max(0,Number(ship.currentShield)||0),c.shieldMax);ship.currentPower=Math.min(Math.max(0,Number(ship.currentPower)||0),c.p.capacity);ship.cargoUsed=Math.min(Math.max(0,Number(ship.cargoUsed)||0),c.cargo);SYSTEMS.forEach(s=>ship.systemDamage[s]=Math.max(0,Number(ship.systemDamage?.[s])||0));}
function render(){
  ...
  <main class="grid">
    <section class="card ship-identity"><div class="card-title"><h2>Ship Identity</h2><span class="pill">Campaign Record</span></div><div class="fields"><div class="field"><label>Ship name</label><input value="${esc(ship.name)}" onchange="setv('name',this.value)"></div><div class="field"><label>Captain / crew lead</label><input value="${esc(ship.captain)}" onchange="setv('captain',this.value)"></div></div></section>
    <section class="card ship-value"><div class="card-title"><h2>Ship Value</h2></div><div class="total">${fmtMoney(c.total)}</div><p class="muted">Hull maintenance: ${fmtMoney(c.maintenance)} / month</p><p class="muted">Hull repair: 100,000 Cr per HP · 1 day per HP</p></section>
    <section class="card initiative-card"><div class="card-title"><h2>Command Points</h2></div><div class="initiative-value"><input type="number" min="0" max="99" value="${ship.commandPoints}" onchange="setn('commandPoints',this.value)"></div><p class="muted">CP / turn · ${esc(ship.bridge)} Bridge · ${c.b.cp} base</p></section>
    <section class="card full"><div class="card-title"><h2>Live Status</h2><span class="pill">Round ${ship.combatRound}</span></div><div class="statgrid"><div class="stat"><small>HULL</small><strong>${ship.currentHull}<i> / ${c.h.hp}</i></strong><button class="mini" onclick="damageHull(1)">−1</button></div><div class="stat"><small>SHIELD</small><strong>${ship.currentShield}<i> / ${c.shieldMax}</i></strong><button class="mini" onclick="damageShield(1)">−1</button></div><div class="stat"><small>POWER</small><strong>${ship.currentPower}<i> / ${c.p.capacity}</i></strong><button class="mini" onclick="spendPower(10)">−10</button></div><div class="stat"><small>CARGO</small><strong>${ship.cargoUsed}<i> / ${c.cargo} t</i></strong><button class="mini" onclick="cargoAdd(1)">+1t</button></div><div class="stat initiative-stat"><small>INITIATIVE</small><strong><input type="number" min="-99" max="99" value="${ship.initiative}" onchange="setn('initiative',this.value)"></strong><i> Bridge initiative</i></div><div class="stat"><small>PASSENGERS</small><strong>${c.passengers}</strong><i> berths</i></div></div><p class="muted hull-spec-line"><b>${esc(ship.hull)} Class</b> · ${c.h.cost} · ${c.h.hp} HP · ${c.h.size} · Shield Factor ${c.h.shieldFactor} · ${c.h.cargo} tonnes cargo · ${c.h.crew} crew quarters · ${c.h.staterooms} staterooms</p></section>
    <section class="card"><div class="card-title"><h2>Core Systems</h2></div><div class="field"><label>Hull</label><select onchange="setCore('hull',this.value)">${options(HULLS,ship.hull)}</select></div><div class="component"><span>${c.h.size} · ${c.h.hp} HP · ${c.h.cargo} t cargo · ${c.h.crew} crew quarters</span><span>${c.h.cost}</span></div><div class="field"><label>Power plant</label><select onchange="setv('powerPlant',this.value)">${options(POWER,ship.powerPlant)}</select></div><div class="component"><span>${c.p.capacity} capacity · ${c.p.size} t · ${c.p.installation} days</span><span>${c.p.cost}</span></div><div class="field"><label>Shield</label><select onchange="setv('shield',this.value)">${options(SHIELDS,ship.shield)}</select></div><div class="component"><span>${c.sh.hp} base HP × ${c.h.shieldFactor} = <b>${c.shieldMax}</b> effective · ${c.sh.physical}P / ${c.sh.energy}E</span><span>${c.sh.cost}</span></div></section>
    <section class="card"><div class="card-title"><h2>Bridge</h2><span class="pill">Upgradeable</span></div><div class="field"><label>Bridge model</label><select onchange="setCore('bridge',this.value)">${options(BRIDGES,ship.bridge)}</select></div><div class="component"><span>${ship.commandPoints} Command Points / turn</span><b>${c.b.cp} base</b></div><div class="component"><span>${ship.initiative} Initiative</span><b>${c.b.initiative} base</b></div><div class="component"><span>${c.b.installation} days installation</span><span>${c.b.cost}</span></div><div class="fields"><div class="field"><label>Command Points (editable)</label><input type="number" min="0" max="99" value="${ship.commandPoints}" onchange="setn('commandPoints',this.value)"></div><div class="field"><label>Initiative (editable)</label><input type="number" min="-99" max="99" value="${ship.initiative}" onchange="setn('initiative',this.value)"></div></div><p class="muted">Changing the bridge automatically resets Command Points and Initiative to the new bridge's base values. You can then edit either value.</p></section>
    <section class="card"><div class="card-title"><h2>Cargo & Crew</h2></div><div class="meter"><span style="width:${Math.min(100,c.cargo?ship.cargoUsed/c.cargo*100:0)}%"></span></div><div class="split"><b>${ship.cargoUsed} t used</b><span>${Math.max(0,c.cargo-ship.cargoUsed)} t free</span></div><p class="muted">Hull cargo: ${c.h.cargo} t · installed modifications and additional power plants occupy ${c.used} t</p><p class="muted">Crew quarters: ${c.h.crew} · Staterooms: ${c.h.staterooms} · Passenger capacity: ${c.passengers}</p><div class="fields"><div class="field"><label>Set cargo used</label><input type="number" min="0" max="${c.cargo}" value="${ship.cargoUsed}" onchange="setn('cargoUsed',this.value)"></div><div class="field"><label>Power recharge</label><button class="btn widebtn" onclick="setn('currentPower',${c.p.capacity})">Fill to capacity</button></div></div></section>
...
function setCore(k,v){ship[k]=v;if(k==='bridge'){const b=BRIDGES[v]||BRIDGES.Basic;ship.commandPoints=Number(b.cp||0);ship.initiative=Number(b.initiative||0)}normalise();render();debouncedSave()}
