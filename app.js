...
function normalise(){if(ship.initiative===undefined||ship.initiative===null||Number.isNaN(Number(ship.initiative)))ship.initiative=Number(BRIDGES[ship.bridge]?.initiative||0);const c=calc();ship.currentHull=Math.min(Math.max(0,Number(ship.currentHull)||0),c.h.hp);ship.currentShield=Math.min(Math.max(0,Number(ship.currentShield)||0),c.shieldMax);ship.currentPower=Math.min(Math.max(0,Number(ship.currentPower)||0),c.p.capacity);ship.cargoUsed=Math.min(Math.max(0,Number(ship.cargoUsed)||0),c.cargo);SYSTEMS.forEach(s=>ship.systemDamage[s]=Math.max(0,Number(ship.systemDamage?.[s])||0));}
function render(){
  ...
  <main class="grid">
    ...
    <section class="card initiative-card"><div class="card-title"><h2>Command Points</h2></div><div class="initiative-value"><strong>${c.b.cp}</strong></div><p class="muted">CP / turn · ${esc(ship.bridge)} Bridge</p></section>
