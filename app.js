...
function render(){
  ...
  <main class="grid">
    ...
    <section class="card initiative-card"><div class="card-title"><h2>Initiative</h2></div><div class="initiative-value"><input type="number" min="-99" max="99" value="${ship.initiative}" onchange="setn('initiative',this.value)"></div><p class="muted">Bridge initiative · modifiable</p></section>
    <section class="card full"><div class="card-title"><h2>Live Status</h2><span class="pill">Round ${ship.combatRound}</span></div><div class="statgrid"><div class="stat"><small>HULL</small><strong>${ship.currentHull}<i> / ${c.h.hp}</i></strong><button class="mini" onclick="damageHull(1)">−1</button></div><div class="stat"><small>SHIELD</small><strong>${ship.currentShield}<i> / ${c.shieldMax}</i></strong><button class="mini" onclick="damageShield(1)">−1</button></div><div class="stat"><small>POWER</small><strong>${ship.currentPower}<i> / ${c.p.capacity}</i></strong><button class="mini" onclick="spendPower(10)">−10</button></div><div class="stat"><small>CARGO</small><strong>${ship.cargoUsed}<i> / ${c.cargo} t</i></strong><button class="mini" onclick="cargoAdd(1)">+1t</button></div><div class="stat"><small>COMMAND</small><strong>${c.b.cp}</strong><i> CP / turn</i></div><div class="stat"><small>PASSENGERS</small><strong>${c.passengers}</strong><i> berths</i></div></div><p class="muted hull-spec-line"><b>${esc(ship.hull)} Class</b> · ${c.h.cost} · ${c.h.hp} HP · ${c.h.size} · Shield Factor ${c.h.shieldFactor} · ${c.h.cargo} tonnes cargo · ${c.h.crew} crew quarters · ${c.h.staterooms} staterooms</p></section>
